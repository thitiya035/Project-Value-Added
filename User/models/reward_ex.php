...
if ($detail_reward['stock_reward'] != 0) {
    if ($detail_user["total_point_user"] >= $detail_reward["point_exchange_reward"]) {
        $db_insert_ex = "INSERT INTO log_exchange_reward(name_user_ex, name_reward_ex, address_user_ex, track_number_ex, status_ex, date_ex, time_ex, id_user) 
    VALUES ('$name_user','$name_reward','$address_user','',1,'$new_startDate','$new_startTime','$id_user')";
        ...
        $remine = [
            "status" => "can't exchange, when point of user < point reward",
            "title" => "Fail!",
            "text" => "Poine is not enough for the exchange reward.",
            "icon" => "error"
        ];
    }
} else {
